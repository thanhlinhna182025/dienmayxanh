...
const PromotionEditScreen = {
  after_render: async () => {
    ...
    // checked radio
    $(async function () {
      const id = $(".title").attr("data-promotion");
      const promotion = await getOnePromotion(id);
      const freeship = promotion.detail.freeship;
    ...
    $("#pr-btn-edit").on("click", async function (e) {
      e.preventDefault();
      const update = {
        name: $("#pr-name").val(),
      ...
      const id = $(".title").attr("data-promotion");
      const res = await updatePromotion(id, update);
      if (res.success) {
        toastMessage({
          type: "success",
          title: "Thành công",
  ...
  render: async () => {
    ...
    <div class="container bgc-edit">
        <div class="left flex_1">
            <h2 class="title" data-promotion="${
              promotion._id
            }">Cập Nhật Chương Trình Khuyễn Mãi ${promotion.name} </h2>
            <div class="fw">
                <form class="form__container fw" id="supplier-form">
                    <div class="form__wrapper">
                        <div class="form__group-item">
                            <input class="fw form__input" type="text" id="pr-name" required value="${
                              promotion.name
                            }"/>
                            <label class="label-position" for="pr-name" >Tên chương trình:</label>
                        </div>
                        <div class="form__group-item">
                            <input class="fw form__input" type="number"  id="pr-discount" required value="${
                              promotion.detail?.discount
                            }"/>
                            <label class="label-position" for="pr-discount" id="pr-discount-label">% Khuyến mãi</label>
                        </div>
                        <div class="form__group-item">
                            <textarea placeholder="Quà tặng kèm nếu có:" class="form__textarea" id="pr-gift"> ${
                              promotion.detail?.gift
                            }</textarea>
                        </div>
                        <div class="form__group-item">
                            <textarea placeholder="Mô tả chi tiết:" class="form__textarea" id="pr-description">${
      ...
      promotion.name
                    ...
                    <div class="btn__container fw">
                        <button class="btn fw " id="pr-btn-edit" type="submit" form="supplier-form">Lưu thông tin</button>
                    </div>
                </form> 
            </div> 
        </div>
